...
1. 写作前 → 调 `load_context`（可选指定角色）**一次**
2. 在对话中直接创作（小说、对白、任务、大纲、本地化等）
3. 完成后 → 调对应的 `save_*` 工具存盘 **一次**

**不要写两份：**
- 不要用 Pi 内置的 `write` / `edit` 保存创作成品，只用 `save_*` 工具
- 不要对同一内容重复调用 save 工具
- 设定与品味通过 `load_context` 加载，不要再用 `read_file` 重复读相同目录

文件保存在**项目根目录**下（如 `参考/`、`作品/`、`角色/`），没有 `vault/` 这一层。
...
**remember_taste** — 追加品味规则到 `品味/taste.md`。

**read_file** — 读任意文本文件（非创作存盘用途）。
...
1. 用户说写 → 先 `load_context`，创作后 `save_*` 一次，主动展示并问要不要改。
